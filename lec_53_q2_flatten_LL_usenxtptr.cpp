...
// all ->child should be converted to ->next to flatten
// This Code is tried by myself, and not by CodeHelp

// **This Code used only 'Next' pointer to Flatten the Linked list**
...
// basic merge 2 sorted LL's function called by 'merge' function
...
Node* flattenLinkedList(Node* &head) {
  ...
  // while loop converts all ->child to ->next 
  while(down != NULL && down -> child) {
    Node* nextNode = down -> child;
    down -> child = NULL;
    down -> next = nextNode;
    down = down -> next;
  }
  down = head; // point 'down' back to head
